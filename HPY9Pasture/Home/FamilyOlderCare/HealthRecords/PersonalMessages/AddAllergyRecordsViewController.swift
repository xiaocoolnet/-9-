//
//  AllergyRecordsViewController.swift
//  HPY9Pasture
//
...
class AddAllergyRecordsViewController: UIViewController,UITableViewDataSource,UITableViewDelegate {

    let myTableView = UITableView()
    let textArray = ["序号","过敏名称","过敏症状","过敏源","过敏史","开始时间","结束时间"]
    var lastText = ["请填写","请填写","请填写","请填写","请填写","请填写","请填写"]
    
    var isbegin = Bool()//是否是开始时间选择
    ...
    func CreatDataPicker(){
        ...
        if self.isbegin {
            datePickerHeaderLabel.text = "开始时间"
        }else{
            datePickerHeaderLabel.text = "结束时间"
        }
...
//            vc.isLongText = true
    ...
    func datePickerDetermineButtonAction(){
        ...
        self.showEffectView()
        if self.isbegin {
            self.lastText[5] = self.briDateString1
            let index = NSIndexPath.init(forRow: 5, inSection: 0)
            self.myTableView.reloadRowsAtIndexPaths([index], withRowAnimation:.Automatic)
        }else{
            self.lastText[6] = self.briDateString1
            let index = NSIndexPath.init(forRow: 6, inSection: 0)
            self.myTableView.reloadRowsAtIndexPaths([index], withRowAnimation:.Automatic)
        }
    }
